...
- **PostgreSQL** — released, up to a declared scope: the table core plus programmable objects. Set `SCHEMORPH_PROVIDER=postgres` and Schemorph inspects, diffs and applies **tables, columns, constraints, indexes, the target schema, and views, functions, procedures and triggers** (the last four re-applied idempotently via `CREATE OR REPLACE`, PostgreSQL's own native idempotent form for all four kinds — see [limitations.md](./docs/limitations.md) for the one case that refuses instead of retrying: a redefinition whose new shape `OR REPLACE` cannot accept). The declarative apply runs as **one transaction** — applied entirely or not at all. Comparison is native `pg_catalog` with shadow normalization ([ADR-0007](./docs/adr/0007-postgres-engine-selection.md)); a database-owner role with `CREATE SCHEMA` suffices — no superuser, no `CREATEDB`, no extensions. Everything outside the declared capability list (versioned migrations, and concurrent index builds) is **refused with an explicit error** rather than half-planned ([ADR-0003](./docs/adr/0003-postgres-as-second-provider.md)); the scope grows in releasable slices with no committed timeline. **If you need versioned migrations on PostgreSQL today, Atlas, sqldef, or Flyway will serve you better** — that is a more useful answer than a date we cannot keep.
