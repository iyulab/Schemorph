...
- **PostgreSQL** — released, at full capability parity with SQL Server's declared surface: tables, columns, constraints, indexes, the target schema, views, functions, procedures, triggers, and versioned migrations. Set `SCHEMORPH_PROVIDER=postgres` and Schemorph inspects, diffs and applies all of it. The four programmable-object kinds are re-applied idempotently via `CREATE OR REPLACE`, PostgreSQL's own native idempotent form for all four (a view whose new column list `OR REPLACE` cannot accept is dropped and re-created instead — refused only when other objects depend on it; see [limitations.md](./docs/limitations.md)). The whole apply — declarative changes, redefines and migrations — runs in **one session** and lands entirely or not at all (`atomicity: "transactional"`; a failure in any stage rolls back everything the apply had done, and the error envelope says so — see [failure-semantics.md](./docs/failure-semantics.md)). SQL Server declares `atomicity: "partial"`: its declarative stage is one transaction, but redefines and migrations commit independently. Comparison is native `pg_catalog` with shadow normalization ([ADR-0007](./docs/adr/0007-postgres-engine-selection.md)); a database-owner role with `CREATE SCHEMA` suffices — no superuser, no `CREATEDB`, no extensions. What remains outside the declared capability list — non-transactional DDL such as `CREATE INDEX CONCURRENTLY` — is **refused with an explicit error** rather than half-planned ([ADR-0003](./docs/adr/0003-postgres-as-second-provider.md)). Parity means equal capability range, deliberately not equal limitations — see limitations.md for where the two engines' behavior genuinely differs.
