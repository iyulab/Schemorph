...
| `1.7` | Adds `messages[].objectName` (additive — the field was reserved as absent/`null` before). A plan-level warning about a specific change (`SCHEMORPH001`, `SCHEMORPH101`–`103`, `SCHEMORPH107`, `SCHEMORPH108`) already named that change in its prose; a consumer could not previously ask "which entry in `changes[]` does this message belong to" without parsing English. `objectName` matches a `changes[].objectName` for these, and stays absent on messages that are not about one object (an engine-level diagnostic, a desired-state file problem). **`planHash` is unchanged** — `messages` was never a hash input (§ Fields, `planHash`); a hash captured under ≤1.6 still matches |
...
```json
{
  ...
  "messages": [
    ...
    { "severity": "Warning", "code": "SCHEMORPH102", "text": "dbo.Category: this change rebuilds the table ...", "objectName": "dbo.Category" }
  ]
}
```
...
| `hasDestructiveChanges` | bool | `true` when any change carries `risk: "destructive"`. It reports what the plan still holds, not what the diff found — so a plan whose destructive change was **gated out** reads `false`, because gating is what removed it from `changes`. A caller that stops on this field alone therefore does not stop on the one plan it most needs to read: the refusal. Gate on `messages[]` carrying `SCHEMORPH001`, or on a non-empty `excluded[]` (since 1.6); this field answers the narrower question of whether a destructive change is about to execute — which is why it pairs with `--allow-destructive`, not with review |
