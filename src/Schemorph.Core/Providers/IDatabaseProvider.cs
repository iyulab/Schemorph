...
public interface IDatabaseProvider
{
    ...
    /// <summary>
    /// Opens the apply-unit transaction boundary this provider's
    /// <see cref="ProviderCapabilities.Atomicity"/> promises, or null when it
    /// promises none (a <see cref="ApplyAtomicity.Partial"/> provider never
    /// returns one). The core opens this once per apply, before the first
    /// execution call, and threads it through every <see cref="ApplyAsync"/>/
    /// <see cref="ExecuteScriptAsync"/> call that follows in the same apply.
    /// </summary>
    Task<IApplySession?> BeginApplySessionAsync(string connectionString, CancellationToken cancellationToken = default);
    ...
    /// </summary>
    Task ExecuteScriptAsync(string connectionString, string script, IApplySession? session = null, CancellationToken cancellationToken = default);
    ...
    /// </summary>
    Task ExecuteScriptAsync(string connectionString, string script, IReadOnlyList<LedgerEntry> ledgerEntries, IApplySession? session = null, CancellationToken cancellationToken = default);
