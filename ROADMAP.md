...
- [ ] **Failure-semantics ADR** — drafted as [ADR-0004](docs/adr/0004-failure-semantics-and-resume.md) (Proposed): convergent re-run, migration/redefine script+ledger in one transaction, transactional declarative publish, failure rows. Decisions 2–4 (defect-class crash-consistency fixes) are implemented and e2e-verified; the marked decision points (resume model wording, cross-strategy ordering remedy) await a human call.
- [ ] **Rollback / down-migration ADR** — drafted as [ADR-0005](docs/adr/0005-rollback-semantics.md) (Proposed): rollback = apply the previous git state; down migrations not supported (roll forward). Awaiting human decision.
